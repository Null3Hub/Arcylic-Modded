...
local function fetchLibrary(url)
    ...
    error(
        "[AcrylicUI] Seu executor nao disponibilizou game:HttpGet/request para baixar a library.",
        2
    )
end
...
local function loadLibrary(url)
    ...
    if type(compile) ~= "function" and type(_G) == "table" then
        compile = rawget(_G, "load")
    end
    if type(compile) ~= "function" then
        error("[AcrylicUI] Seu executor nao disponibilizou loadstring/load.", 2)
    end

    local source = fetchLibrary(url)
    local chunk, compileError = compile(source)
    if type(chunk) ~= "function" then
        error("[AcrylicUI] Falha ao compilar src.lua.txt: " .. tostring(compileError), 2)
    end

    local ok, libraryOrError = pcall(chunk)
    if not ok then
        error("[AcrylicUI] Falha ao inicializar src.lua.txt: " .. tostring(libraryOrError), 2)
    end
...
local autoFarmToggle = actionsSection:Toggle({
    Name = "Auto Farm",
...
local damageSlider = actionsSection:Slider({
...
local targetDropdown = actionsSection:Dropdown({
...
local weaponsDropdown = actionsSection:Dropdown({
...
local walkSpeedSlider = movementControls:Slider({
    Name = "Walk Speed",
...
local flyKeybind = movementControls:Keybind({
    Name = "Fly Toggle",
...
local teleportBox = movementControls:TextBox({
...
local accentPicker = themeSection:ColorPicker({
    Name = "Accent Color",
...
local statusBox = themeSection:TextBox({
    Name = "Status Message",
...
local infoParagraph = themeSection:Paragraph({
    Title = "Library Info",
...
local runtimeSection = runtimeTab:CreateSection({
